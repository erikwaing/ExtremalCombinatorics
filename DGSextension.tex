...
This proof gives a procedure that either gives a family $\F$ that satisfies the desired properties, or decreases $\Phi$ by at least 1 (by decreasing $\A$ or $r$) without increasing the number of 2-chains. Since $\Phi \geq 0$, repeating the procedure guarantees taht eventaully we will have a family with the desired properties.
...
\begin{lemma} For any set $\F$ that does not satisfy the properties in Proposition~\ref{proposition1},
\label{lemma1}
\[ \sum_{A\in \A}\sum_{i = 1}^{l-1} |U^i(M(A)) \cap \F| \leq \sum_{A \in \A}\sum_{i = 1}^{l-1} |\partial^i A | \]
when $l \leq 2r-1$
\end{lemma}

\begin{proof} Let $L$ be the expression on the left-hand side, and let $R$ be the expression on the right-hand side. 
