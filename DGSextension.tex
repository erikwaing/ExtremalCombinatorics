...
%
%	Here begins comment of proposition 2.
%	It will make it super easy to prove Property 2 of the theorem I think.
%	Have yet to make it work, although I believe it comes down to algebra.
%
%
\begin{comment}
...
\[ \sum_{i=1}^{2r-1} \dbinom{\frac{n}{2} +r-1}{i-1} + \dbinom{\frac{n}{2} + r-2}{i-2} \]
...
%
%	Unsure about these next two....
%
%

3) If $s > \sum_{i=\frac{n}{2}-r+1}^{\frac{n}{2}+r}$, then the level $\frac{n}{2}+r$ is full. In the $\frac{n}{2}-r$ level, elements without $\{1, 2\}$ are prioritized.

4) If $s \leq \sum_{i=\frac{n}{2}-r+1}^{\frac{n}{2}+r}$, then the level $\frac{n}{2}-r$ is empty. In the $\frac{n}{2} + r$ level, elements with $\{1, 2\}$ are prioritized.

then it minimizes the number of 2-chains.
\end{theorem}
...
For the inductive step, take a family of sets $\F_0$ with $s-1$ elements that satisfies the properties and thus minimizes the number of 2-chains for $s-1$ elements. First, we analyze the case where the $\frac{n}{2}+r$ level is not full, so we add a set $F_0$ to the level $\frac{n}{2} + r$. Note that $\F_0 \cup \{F_0\}$ still satisfies properties 1 through 4.

We will assume that there exists a different family with fewer 2-chains and show that if one of the properties is broken, there isn't less 2-chains than in $\F_0 \cup \{ F_0\}$. Therefore, this will show that there exists a family of sets which satisfies the above properties and minimzes the number of 2-chains. However, all the families of sets satisfying these properties are the symmetric. Which will give us the theorem.
...
Now suppose a family $\F$ of $s > m$ sets that has fewer 2-chains than $\F_0 \cup \{F_0\}$. Apply the transformations necessary to get a family that satisfies Proposition~\ref{proposition1} and minimizes the number of 2-chains. Suppose this family does not satisfy Property 1. Since $\F$ and $\F' = \{ [n] - F | F \in \F \}$ are involved in the same number of 2-chains, we can assume that the largest element $F \in \F$ has $|F| = \frac{n}{2} + t$ where $t > r$. 
...
2-chains since the $\frac{n}{2}-r$ level is empty andwe are dealing with Case 1. Since $t > r$, we know that $\F$ has more 2-chains than $\F_0$, which means that $\F$ must satisfy Property 1.
...
Suppose $\F$ satisfied property 1 but not property 2, then we know that there is an element in level $\frac{n}{2} \pm r$ and an element missing inside in level $\frac{n}{2}+r-l$. We can move the elements down via Proposition~\ref{proposition2}. Until we get property 2. 
