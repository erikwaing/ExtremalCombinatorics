...
\B &= \partial^l A - \F
\end{align*}
The proof will give a procedure by which to either decrease $|\A|$, decrease $r$ or increase $l$. Since $l \leq 2r-1$ and $r\geq 1$ and $|\A|>0$, this procedure will eventually terminate with a family satisfying the properties of the proposition.
...
\begin{corollary}
There exists a family of $s > m$ subsets satisfying the properties in Proposition~\ref{proposition1} and is such taht if $A \in \F$ is of minimal cardinality, where we write $|A|=\frac{n}{2}-r$, then for any $A \subset B$ with $|B| \leq \frac{n}{2}+r-1$, $B \in \F$
\end{corollary}
