...
Let $l \leq 2m-1$ be the smallest integer such that there exists a maximal cardinality set $A$ with a corresponding subset $l$ levels below that is not in $\F$. We define
\begin{align*}
...
Suppose $l \geq 1$. By the minimality of the choice of $l$, all the shadows from $\A$ that are less than $l$ must be in $\F$. That is $\partial^i A \subset \F$ for $1 \leq i < l -1$. We will count the number of intermediate 2-chains that we lose and gain in order to show that we lose more 2-chains that we gain.
...
%%following part is almost a direct copy paste from pluginproof.tex and needs to be completely changed!! only included for now so everything is in one place
%%%%
%%%
%%
%%		revise up to here.
...
If $X \subset \A - \{ A \}$, then each element in $X$ has at least $\frac{n}{2} + r - 1$ outgoing edges. Furthermore, each element $B \in \B'$ has at most $\frac{n}{2} - r + 1$ outgoing edges. So if $U \subset X$, and we let $E$ be the number of edges,
\[ |U|*(\frac{n}{2}+r-1) = E \leq (\frac{n}{2}-r+1)|N(U)| \]
Since $r \geq 1$, $\dfrac{\frac{n}{2}+r-1}{\frac{n}{2}-r+1} \geq 1$ so we can apply Phillip Hall's theorem.

In the shifted set, we have lost the 2-chain $C \subset A$ and gained no intermediate 2-chains. So the shifted set has fewer 2-chains. Therefore, the sets in $\A$ cannot be part of any 2-chains. So the shifted set to $\partial \A$ will not be in any 2-chains. Since $|\F|$ is greater than the middle level, we know that there is some 2-chain, suppose it is $C \subset D$. So we can additionally shift $D$ to some element in $\partial \A$. So we lose a 2-chain and not gain any. 

So that means that there must not be a matching from $\A \rightarrow \B$. 

%%end of direct copy paste part
...
For the inductive step, assume $s > m$. Consider the family $\F$ that satisfies the property in Proposition~\ref{proposition1}, whose existence is guaranteed by the proposition. We claim this family $\F$ satisfies the two desired properties.
